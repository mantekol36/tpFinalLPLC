...
static void toString_MesaImpl(void *self)
{
   	...
   	obj_Localidad *loc=escuela->getLocalidadObj(escuela);
	obj_Circuito *circuito=self_o->getCircuitoObj(self_o);
    obj_Seccion *seccion=circuito->getSeccionObj(circuito);
	// obj_Seccion *seccion=self_o->getSeccionObj;
	 printf("Mesa nro: %d  Nro De Mesa:%d  Escuela:%s Localidad:%s Circuito:%s Seccion:%s \n ",
	 self_o->info.Mesa_id,
	 self_o->info.nro_Mesa,escuela->info.nombre_Escuela,
	 loc->getNombreLocalidad(loc),
	 circuito->info.nombre_Circuito,seccion->getNombreSeccion(seccion));
}
//----------------------------------------------------
